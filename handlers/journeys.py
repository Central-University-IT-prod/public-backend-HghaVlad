...
from base_req import get_user_journeys, get_journeys_by_traveller, make_journey, claim_journey, \
    ...
    get_user_name, get_user_telegram_id, delete_note
...
from keyboards import cancel_keyboard, journey_menu_keyboard, confirm_keyboard, main_menu_keyboard, \
    ...
    expenses_keyboard, expenses_journey, expenses_back_keyboard, debts_back_keyboard, journey_notes_delete_keyboard
...
async def journeys_callback(call: CallbackQuery):
    ...
    elif call.data == "journey_notes_delete":
        await call.message.edit_text("<b>Вы уверены, что хотите удалить заметку?</b>", reply_markup=journey_notes_delete_keyboard)
    elif call.data == "journey_delete_note_yes":
        note = user_journey_data[call.message.chat.id]["notes"][user_journey_data[call.message.chat.id]["notes_step"]]
        delete_note(note.id)
        await call.message.delete()
        await call.message.answer("<b>Заметка успешно удалена</b>")
    elif call.data == "journey_delete_note_no":
        await see_note(call.message)
...
async def new_journey_description(message: Message, state: FSMContext):
    ...
    await message.answer("<b>Сколько локаций вы хотите посетить?</b>")
    await NewJourney.next()
...
async def new_journey_description(message: Message, state: FSMContext):
    if message.text.isdigit():
        if 1 <= int(message.text) <= 16:
            ...
            return
    await message.answer("<b>Кол-во локаций должно быть целым числом от 1 до 16</b>")
